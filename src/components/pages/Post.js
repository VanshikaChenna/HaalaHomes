...
export default function UserProfile_post() {
    ...
    const changeToIST=(timeStamp) =>{
      ...
      const fetchComments = async () => {
        try {  
        const commentsFetch = await api.get(`posts/comments/${postId}`)
    ...
    <Navbar />
        <div className='w-screen md:w-full mb-40 p-10 md:p-20 justify-center'>
            <div className='md:flex w-full md:space-x-10 justify-center'>
               <div className=' w-3/5 flex space-y-10 flex-col'> 
                <div className='p-10 w-full  shadow-lg'>
                    <div className='flex space-x-4'>
                        <img src={profilePic_1} className='h-10 w-10 rounded-full object-cover '></img>
                        <div className='flex flex-col space-y-8'>
                            <div>
                                <h2 className='text-2xl'>{post.question}</h2>
                                <p className=' text-sm font-bold'>{post.full_name} | <span className='text-gray-400'>{changeToIST(post.created_at)}</span></p>
                            </div>
                            <div>
                        ...
                        <div className='flex flex-col w-full pr-20 items-ceter space-y-8'>
                                <div className='flex w-full justify-between'>
                                    <div>
                                        ...
                                        <p className=' text-sm font-bold'>{comment.full_name} | <span className='text-gray-400'>{changeToIST(comment.created_at)}</span></p>
                                    </div>
                                    {isOpen && (
                                        <div>
                        ...
                        (
                          ...
                          (
                            ...
                            <p className='pl-2 text-black'>{comment.like_count}</p>
                                    ...
                                    0
                                    <Link to={`/portfolio/?user_id=${comment.user_id}`} className='rounded-full px-3 p-1 bg-[#994b002a] text-[#994b00]'>
                                        Show Portfolio
                                    </Link>
                                </div>
                    </div>
...
these marks on their quartzite?</p>
    ...
    <Modal isvisible ={showModal} onClose={() =>setShowModal(false)} full_name={selectedFullName} user_id={selectedUserId}/>
    </Fragment>
  );
}
